...
void setInterruptPriority(unsigned char device, unsigned char priority) {
    switch (device) {
        ...
        case INT2_DEVICE:
            ...
            break;
        case INT3_DEVICE:
            INT3_PRIORITY = priority;
            break;
        case INT4_DEVICE:
            INT4_PRIORITY = priority;
            break;
        case CN_DEVICE:
             CN_PRIORITY = priority;
             break;
        case TIMER1_DEVICE:
...
void switchExternalInterruptEdge(unsigned char device) {
    switch (device) {
        ...
        case INT2_DEVICE:
            ...
            break;
        case INT3_DEVICE:
            if (INT3_TRIGGER_EDGE == NEGATIVE_EDGE) {
                INT3_TRIGGER_EDGE = POSITIVE_EDGE;
            } else {
                INT3_TRIGGER_EDGE = NEGATIVE_EDGE;
            }
            break;
        case INT4_DEVICE:
...
/**
   \param timePeriod Period in seconds.
*/
...
/**
   \param timePeriod Timer period in seconds.
*/
double getExactTimerPrescaler(double timePeriod) {
       // Convert time in microseconds, then divide by maximum measurable period without prescaler.
    return (timePeriod * 1000000) / (INSTRUCTION_PERIOD * MAX_TIMER_COUNTER_VALUE);
}
